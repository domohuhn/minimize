...
#include "minimize/steepest_descent.hpp"
...
SCENARIO("Steepest descent: fit linear function", "[steepest descent]") {
    GIVEN("Perfect measurement data") {
        ...
        WHEN("the minimum is searched") {
            const auto found = steepest_descent(linear, vec, 1.0e-15);
            THEN("a minimum is found") {
                const auto chi2 = compute_chi2(linear, vec);
...
SCENARIO("Steepest descent: gaussian function", "[steepest descent]") {
    GIVEN("Perfect measurement data") {
        Gaussian gauss{};
        MeasurementVector<1> vec{};
        for (size_t i = 0; i < 100; ++i) {
        ...
        WHEN("the minimum is searched") {
            const auto found = steepest_descent(gauss, vec, 1.0e-15);
            THEN("a minimum is found") {
                const auto chi2 = compute_chi2(gauss, vec);
...
SCENARIO("Steepest descent: fit saddle function", "[steepest descent]") {
    GIVEN("Perfect measurement data") {
        ...
        WHEN("the minimum is searched") {
            const auto found = steepest_descent(saddle, vec, 1.0e-9);
            THEN("a minimum is found") {
                // there are infinitely many solutions
                // => check that chi2 converged.
                const auto chi2 = compute_chi2(saddle, vec);
                REQUIRE_THAT(chi2, Catch::Matchers::WithinAbs(0.0, 1e-18));
            }
        }
    }
